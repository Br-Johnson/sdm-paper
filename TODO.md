...
- [x] Add a short distinction between **Data Management Plans** vs **Data Product Governance Charters** vs **Data Sharing Agreements** (2–6 sentences or a small box).
- [x] Clarify and standardize the terms **data lifecycle** vs **salmon lifecycle** (define once; use consistently).
- [x] Improve the **first mention / placement** of the Best Practices table (make it obvious how to use @tbl-bestpractices when it is first referenced).
...
- [x] Decide whether to remove/compress the “Data stewardship as a sub-discipline of data science” paragraph (keep if useful; compress if it reads like a detour for *Fisheries*).
...
- [x] Consider renaming Best Practices table right column from “practical applications” → “next steps (examples)” (ultra-light table tweak).
...
- [x] Appendix 1: fill BP 5–7 more thoroughly (aim: +1–2 additional examples each).
- [x] Appendix 2: refine/reorg “training roadmap” (keep content, but add Beginner/Intermediate/Advanced labels; weave in SDIS resources as salmon-domain examples).

### Journal boilerplate / compliance (target: *Fisheries*)

- [x] Fill Acknowledgements (brief, accurate).
- [x] Add Data availability + Code availability statements (as required/appropriate).
...
- [x] PSF/Bruno/Steph/Eric: second pass on remaining docx comments that affect flow/structure/word choice (do after you review current branch).
