...
|---|---|
| **1. Make Data Governance Foundational and Explicit to Establish and Sustain Trust and Reuse.** Establishing clear governance structures promotes quality, accountability, and alignment with FAIR and CARE principles enabling trust and long-term stewardship across multi-organization projects. | • Document roles and responsibilities using a Data Product Governance Charter and structured responsibility frameworks.<br>• Integrate CARE principles to respect First Nations, Tribes, and Indigenous data rights.<br>• Form a governance or oversight committee to review data standards, timelines, and agreements. |
| **2. Reuse Proven Infrastructure to Save Time and Increase Interoperability.** Leveraging existing platforms and technologies by building on and extending them rather than building bespoke solutions reduces costs and improves long-term interoperability and sustainability. | • Use domain-specific repositories.<br>• Publish and archive data with established repositories (e.g., KNB, Zenodo) matched to your data type and governance context. |
| **3. Make Data, People, Projects, and Outputs Discoverable, Linked and Citable with Persistent Identifiers.** Persistent identifiers (PIDs) connect data with researchers, organizations, and outputs—supporting data citation, reuse, and automated attribution. | • Encourage ORCID use for researchers.<br>• Use ROR IDs for organizations.<br>• Assign DOIs via DataCite for data packages.<br>• Embed DOIs in dashboards and metadata. |
...
| **5. Store and Analyze Data in Ways That Others Can Easily Access, Use and Trust.** Structured and accessible data formats ease reusability and integration with analytical tools, while programmatic workflows (e.g., R, Python) increase reproducibility and trust. | • Provide programmatic access (e.g., APIs) where appropriate.<br>• Archive in trusted repositories (e.g., GBIF, FRDR, USGS).<br>• Use scripts to wrangle, transform, and analyze data reproducibly.<br>• Use GitHub for transparent collaboration and GitHub/Zenodo for DOI assignment and preservation. |
...
SDIS (Salmon Data Integration System) is a worked example of how the seven best practices can be implemented together in a salmon monitoring context. It is not a prescribed software stack; it is a transferable pattern: make governance and access conditions explicit, package data with standardized machine-readable metadata, use controlled terminology and persistent identifiers, validate automatically, and publish versioned, citable releases.
...
-   **[Salmon Domain Ontology](https://github.com/salmon-data-mobilization/salmon-domain-ontology/)** (canonical interoperability layer for shared salmon concepts)
-   **DFO Salmon Ontology** [@dfo_salmon_ontology] (<https://dfo-pacific-science.github.io/dfo-salmon-ontology/>) (organization-specific ontology and controlled vocabularies aligned to the broader [Salmon Domain Ontology](https://github.com/salmon-data-mobilization/salmon-domain-ontology/) layer)
-   **Salmon Data GPT** [@smn_gpt] (<https://github.com/dfo-pacific-science/smn-gpt>)

Other programs can implement the same stewardship pattern using different tools. Where provenance is important, relationships among datasets, methods, and releases can be expressed using standard conceptual models (e.g., PROV-DM) [@w3c_prov_dm_2013].
:::

In SDIS, governance, metadata, packaging, validation, and semantics are treated as one operational workflow to support living datasets that remain interpretable and reusable across time, teams, and institutions.

For teams that use lifecycle framing, these practices operate across the full data lifecycle, with metadata governance treated as a continuous cross-cutting function rather than a separate step.
